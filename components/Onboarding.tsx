...
const ProfileIntake: React.FC = () => {
  ...
  const [isEditing, setIsEditing] = useState(!state.userProfile); // if no profile, start in edit mode
  ...
  const profileCompleteness = useMemo(() => {
    const fieldsToTrack: (keyof Omit<UserProfile, "email" | "language">)[] = [
      "name",
      "stream",
      "interests",
      "goals",
    ...
    const filledFields = fieldsToTrack.reduce((count, field) => {
      ...
      return (
        count + (value && typeof value === "string" && value.trim() !== "" ? 1 : 0)
      );
    }, 0);
    return Math.round((filledFields / totalFields) * 100);
  }, [formData]);
  ...
  const formInputClasses =
    "block w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm placeholder-slate-400 dark:placeholder-slate-500 bg-white dark:bg-slate-800 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm disabled:bg-slate-50 dark:disabled:bg-slate-700";
    ...
    <div className="bg-white dark:bg-slate-800 max-w-2xl mx-auto my-12 p-6 md:p-10 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
      <div className="text-center mb-8">
        <h2 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-white">
          {isEditing ? "Complete Your Profile" : "Your Profile"}
        </h2>
        <p className="text-slate-500 dark:text-slate-400 mt-2">
          {isEditing
            ? "This helps our AI understand your background and aspirations."
            : "Here’s a summary of your profile details."}
        </p>
      </div>

      {/* Progress Bar (show only in edit mode) */}
      {isEditing && (
        <div className="mb-8 px-2">
          <div className="flex justify-between items-center mb-1">
      ...
      {!isEditing && (
        <div className="space-y-6">
          <div className="bg-slate-50 dark:bg-slate-700/30 p-5 rounded-lg">
          ...
          <button
            ...
            className="w-full mt-4 py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition-colors"
          >
            Edit Profile
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <fieldset disabled={isLoading}>
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                className={formInputClasses}
                placeholder="e.g., To work at a product-based company, specialize in AI..."
                required
              />
            </div>
          </fieldset>
          ...
          <div className="pt-4 flex gap-3">
            <button
              type="submit"
              disabled={isLoading}
              className="flex-1 py-3 px-4 rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors disabled:bg-indigo-400"
            >
            ...
            <button
              ...
              className="flex-1 py-3 px-4 rounded-md border border-slate-300 dark:border-slate-600 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
            >
              Cancel
            </button>
          </div>
