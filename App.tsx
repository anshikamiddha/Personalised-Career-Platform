...
const Header: React.FC = () => {
  ...
  const handleLogin = () => {
      // Redirect to AuthPage by resetting to logged-out state
      ...
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navLinks.map(({ page, label }) => (
             ...
             <ThemeToggle />
             <button onClick={handleLogin} className="px-3 py-2 rounded-md text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">Login</button>
        ...
        {isMenuOpen && (
            <div className="md:hidden pt-2 pb-4 space-y-1">
                ...
                <div className="border-t border-slate-200 dark:border-slate-700 pt-4 mt-4 grid grid-cols-2 gap-3">
                    <button onClick={handleLogin} className="text-center px-3 py-2 rounded-md text-base font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">Login</button>
                    <button onClick={handleLogout} className="text-center px-3 py-2 rounded-md text-base font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">Logout</button>
                </div>
            </div>
        )}
      </nav>
